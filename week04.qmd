...
The : operator creates integers `c()` combines values into a vector `typeof()` shows internal storage, `class()` shows object class
...
The L suffix makes it into an integer.
Mixing types results in coercion to the most flexible type as.integer() explicitly converts to integer type.
...
In R, paste() and cat() are both used for string concatenation, but they differ in how they handle the output.
paste() creates a character string, which can be stored in a variable, while cat() displays the concatenated string directly to the console and doesn't return a value.
The glue() function, from the glue package, provides a more convenient way to embed variables within strings using `{}` placeholders.
...
The expression 1 \<- 1 in R will cause an error because 1 is a numeric literal and cannot be assigned a value.
...
Factors represent categorical data Stored as integers with labels (levels) Useful for statistical analysis and plotting `table()` counts occurrences of each level.
...
The first matrix fills the numbers in by columns and the second fills them in by row.
R fills in by columns as its default.
...
It was missing the quotation around the word sea on the second one.
...
The proportion depends on how many people were born in that year as well where as the number is just the true number of people named that name in the year.
...
cholesterol_data_longer <-  cholesterol_data |> 
  pivot_longer(
  cols = c('oat_bran_free', 'oat_bran'),
  names_to = "diet",
  values_to = "cholesterol"
)
cholesterol_data_longer
...
cholesterol_data_longer |> 
  ...
  summarise(
    n = n(),                       # Sample size
    mean = mean(cholesterol),      # Average cholesterol
    sd = sd(cholesterol),          # Standard deviation
    .groups = "drop" # experimental feature, similar to ungroup()
  )
